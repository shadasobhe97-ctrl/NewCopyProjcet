...
class _ParentAlternativePhoneScreenState extends State<ParentAlternativePhoneScreen> {
  ...
  void _goToEmailScreen({bool isSkipped = false}) {
    ...
    // حفظ رقم الهاتف البديل في الـ Cubit (اختياري)
    cubit.alternativePhone = isSkipped ? null : _altPhoneController.text.trim();

    // الخطوة 3 → 4: الانتقال لشاشة الإيميل (إرسال OTP)
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          // زر تخطي (اختياري)
          TextButton(
            onPressed: () => _goToEmailScreen(isSkipped: true),
            child: Text(
              "تخطي",
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                ...
                // زر التالي → الانتقال لشاشة الإيميل
                ElevatedButton(
                  onPressed: () {
                    if (_formKey.currentState!.validate()) {
                      _goToEmailScreen(isSkipped: false);
                    }
                  },
